...
interface SimpleChatbotSidebarProps {
  onSelectPrompt?: (promptText: string) => void;
}

const PINNED_STORAGE_KEY = 'nexus_pinned_simple_chat_ids';
const CONV_CACHE_KEY = 'nexus_simple_chat_conv_cache';

export const SimpleChatbotSidebar: React.FC<SimpleChatbotSidebarProps> = () => {
  const {
  ...
  const loadConversations = async () => {
    try {
      ...
      console.debug('Failed to load simple chat conversations:', err);
    }
  };
  ...
  const handleSelectConversation = async (convId: string) => {
    ...
    // Clear current messages instantly so stale content doesn't flash
    setMessages([]);
    try {
  ...
  const handleSaveRename = async (convId: string) => {
    ...
    try {
      await api.updateConversation(convId, trimmed);
      setConversations((prev) =>
        prev.map((c) => (c.id === convId ? { ...c, summary: trimmed } : c))
      );
    } catch (err) {
      console.error('Failed to rename simple conversation:', err);
    } finally {
      setRenamingConvId(null);
    }
  };
  ...
  const handleDeleteConversation = async (convId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      console.error('Failed to delete simple conversation:', err);
    } finally {
      setMenuOpenConvId(null);
    }
  };

  // Only show Simple Chatbot conversations (non-[Computer-Use])
  const simpleConversations = conversations.filter((c) => !(c.summary || '').startsWith('[Computer-Use]'));

  // Sort conversations: Pinned first, then by date
  const sortedConversations = [...simpleConversations].sort((a, b) => {
  ...
  return (
    <aside className="w-64 glass-panel rounded-none border-t-0 border-l-0 border-b-0 border-r border-cyan-500/20 flex flex-col justify-between py-4 px-2 select-none bg-slate-950/85 backdrop-blur-2xl shrink-0 h-full">
      {/* Top Header: CHAT HISTORY + Search Icon OR Full-width Transparent Search Bar */}
